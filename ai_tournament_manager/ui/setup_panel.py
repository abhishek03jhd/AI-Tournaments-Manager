"""SetupPanel — participant entry form, algorithm selector, seeding mode."""
...
from ai_tournament_manager.domain.validators import (
    ValidationError,
...
class SetupPanel(tk.Frame):
    """Form for configuring a tournament before it starts."""

    def __init__(
        self,
        master: tk.Widget,
        on_create: Callable[[list[Participant], AlgorithmType, SeedingMode], None],
        **kwargs,
    ) -> None:
        super().__init__(master, **kwargs)
        self._on_create = on_create
        self._rows: list[dict] = []  # each dict: {name_var, attr_vars[]}
    ...
    def _build(self) -> None:
        tk.Label(self, text="AI Tournament Manager", font=("Helvetica", 16, "bold")).grid(
            row=0, column=0, columnspan=6, pady=(10, 4)
        )
        ...
        ttk.Combobox(
            algo_frame, textvariable=self._algorithm_var,
        ...
        ttk.Combobox(
            seed_frame, textvariable=self._seeding_var,
            values=_SEEDINGS, state="readonly", width=15
        ).pack(side=tk.LEFT, padx=4)

        # Error label
        tk.Label(self, textvariable=self._error_var, fg="red", wraplength=400).grid(
            row=6, column=0, columnspan=6
        )

        # Create button
        self._create_btn = tk.Button(
            self, text="Create Tournament", command=self._on_create_clicked,
            bg="#4CAF50", fg="white", font=("Helvetica", 11, "bold")
        )
        self._create_btn.grid(row=7, column=0, columnspan=6, pady=8)
    ...
    def _add_row(self) -> None:
        ...
        remove_btn = tk.Button(frame, text="✕", fg="red",
                               command=lambda r=row_idx: self._remove_row(r))
        remove_btn.grid(row=row_idx, column=4, padx=2)

        self._rows.append({"name_var": name_var, "attr_vars": attr_vars, "frame_row": row_idx})
    ...
    def _on_create_clicked(self) -> None:
        ...
        algorithm = AlgorithmType(self._algorithm_var.get())
        seeding = SeedingMode(self._seeding_var.get())
        self._on_create(participants, algorithm, seeding)
